...
|---|---|---|---|
| **Physical Sign-Up Sheet** | Printed 8.5×11" landscape paper sheet, one per location | Primary shop-floor touchpoint for Members; hosts the task list, sign-off fields, and QR code; serves as the OCR input artifact when photographed | Member (reads/writes on it), QR Code (embedded in it), Claude AI Model (photographed for OCR) |
...
| **Mobile Phone** | Member's personal smartphone — no app installation required | Uses the native camera to scan a QR code; the phone's browser opens automatically to the Mobile Web Form (or Slack). No app, no account, no login required. May also photograph a completed sign-up sheet for OCR upload | QR Code (scans), Mobile Web Form (browser opens to), Slack (optional NL queries) |
| **Mobile Web Form** | Lightweight mobile-optimized web app, served when a QR code is scanned | Presents the task list for a location; collects member name and selected task; POSTs the completion record to the Agentic Workers; designed for ≤ 2 taps from scan to confirmation | Mobile Phone (rendered on), Agentic Workers (POST target for completion records) |
...
| **Claude AI Model** | Anthropic LLM (`claude-sonnet-4-6`), invoked via API by Agentic Workers | Provides two capabilities: vision-based OCR (extract completion data from sign-up sheet photos) and natural language query processing (interpret plain-English questions from Members) | Agentic Workers (invoked by), Physical Sign-Up Sheet (processes photos of) |
...
| **Agentic Workers** | Python Slack bot built with the Slack Bolt framework | Listen to Slack events; dispatch commands to the correct pipeline; invoke the Claude AI Model for OCR and NL tasks; read from and write to Google Sheets; route notifications to the correct channels | All human actors (via Slack), all Slack channels, Google Sheets, Claude AI Model, Mobile Web Form, APScheduler |
...
* No app may be installed on a Member's mobile device — the entire digital interaction must be accessible through the phone's native camera and browser
...
| **Mobile app** | To reduce friction, it is a requirement that there is no need for a native iOS or Android app. The mobile experience is delivered through Slack and a mobile-optimized web form (Method 3). |
