...
<style>
  ...
  th, td {{ border: 1px solid #888; padding: 1px 6px; line-height: 1.2; vertical-align: top; }}
...
<div class="page" style="page-break-after: always; padding: 0.3in;">
  <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #333; padding-bottom:4px; margin-bottom:6px;">
    <div style="flex:0 0 170px; text-align:center;">
      {{% if qr_b64 %}}
        <img src="data:image/png;base64,{{{{ qr_b64 }}}}" width="160" height="160" alt="QR">
      {{% endif %}}
    </div>
    <div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td class="col-task" style="position:relative; padding-bottom:14px;"><span style="color:red; font-weight:bold;">{{{{ task.name }}}}</span>{{% if task.instructions %}}<br><span style="font-size:8pt; font-weight:normal;">{{{{ task.instructions }}}}</span>{{% endif %}}{{% if task.task_id %}}<div style="position:absolute; bottom:1px; left:6px; font-size:8pt; font-weight:normal;">{{{{ task.task_id }}}}</div>{{% endif %}}</td>
